//! Runtime observer for Xenia Ready Compiler and graphics evidence.
//!
//! Static stage facts and verdict vocabulary come from the selected route
//! package. This file owns mutable, live log-observation state.

const std = @import("std");
const static_evidence = @import("xenia_startup_evidence");

pub const host_architecture = static_evidence.host_architecture;
pub const host_codegen = static_evidence.host_codegen;
pub const Stage = static_evidence.Stage;
const ordered_stages = static_evidence.ordered_stages;
pub const required_stage_count = static_evidence.required_stage_count;
pub const stageFromName = static_evidence.stageFromName;
pub const Verdict = static_evidence.Verdict;

fn bit(stage: Stage) u64 {
    return static_evidence.stageBit(stage);
}
...
test "package identity is preserved by the runtime adapter" {
    try std.testing.expectEqualStrings(static_evidence.host_architecture, host_architecture);
    try std.testing.expectEqualStrings(static_evidence.host_codegen, host_codegen);
}
...
test "the supplied x86 frontier is classified without forcing it" {
    ...
    report.ingest(
        "READY COMPILER: package HIT path=.rosette/packages/xenia-halo3-ready.r3pkg\n" ++
            ...
            "READY COMPILER: CONTRACT PROGRESS block=21/34 required=21/34 edges=21/36\n" ++
    ...
    try std.testing.expectEqual(@as(u8, 34), report.contract_blocks_total);
