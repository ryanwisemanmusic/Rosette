...
const bat = @import("root.zig");

const BatTranslator = bat.BatTranslator;
const BatError = bat.BatError;

fn usage(exe_name: []const u8) void {
    std.debug.print(
        \\Translate Windows .bat scripts to POSIX shell or Makefile.
        \\
        \\Usage:
...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len < 2) {
        usage(if (args.len > 0) args[0] else "bat_processor");
        std.process.exit(1);
    }
    ...
    var translator = BatTranslator.init(allocator);
    ...
    if (mode_dry_run) {
        ...
        try stdout_w.writeAll(out_buf.items);
