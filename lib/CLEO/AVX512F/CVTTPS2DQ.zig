...
pub const meta = types.InstructionMeta{
    .name = "CVTTPS2DQ",
    .family = "CONVERT",
    .source_path = "ISA/x86/CONVERT/CVTTPS2DQ.inc",
    .required_feature = .avx512f,
    .max_width_bits = 512,
    .element_bits = 32,
    .operation = .cvtt_ps2dq,
    .alignment = .any,
...
pub fn unaryMasked(comptime bits: usize, merge: wide.Wide(bits), src: wide.Wide(bits), mask: u64, mode: wide.MaskMode, features: types.FeatureSet) types.SafetyError!wide.Wide(bits) {
    return instruction.unaryMasked(bits, meta, merge, src, mask, mode, features);
}
