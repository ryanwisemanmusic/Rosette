...
pub const meta = types.InstructionMeta{
    .name = "VPMOVSDW",
    .family = "DOWN_CONVERT",
    .source_path = "ISA/x86/DOWN_CONVERT/VPMOVSDW.inc",
    .required_feature = .avx512f,
    .max_width_bits = 512,
    .element_bits = 32,
    .operation = .down_convert_signed,
    .alignment = .any,
...
pub fn unary(comptime bits: usize, src: wide.Wide(bits), features: types.FeatureSet) types.SafetyError!wide.Wide(bits) {
    return instruction.downConvert(bits, meta, src, u32, u16, .signed_saturate, features);
}
