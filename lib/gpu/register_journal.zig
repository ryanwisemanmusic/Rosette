...
/// Functional blocks of the Xenos register file, by index range.
///
/// The ranges come from the register map the executor already writes through.
/// A write outside every known block is `unclassified` rather than being
/// forced into the nearest one: an unexpected index is evidence, and rounding
/// it away is how a decode defect hides.
pub const Block = enum(u8) {
    /// Command-processor control: ring pointers, program counter.
    command_processor,
    /// Render backend: colour/depth targets, surface, blend, masks. The block
    /// that decides where pixels land.
    render_backend,
    /// Rasteriser and viewport setup.
    raster_setup,
    /// Vertex/index fetch and draw initiation.
    geometry,
    /// Shader program control and export setup.
    shader_control,
    /// Shader constant registers: float, boolean, loop.
    shader_constants,
    /// Texture and vertex fetch constants.
    fetch_constants,
    /// Anything outside the known blocks.
    unclassified,
    ...
    pub fn meaning(self: Block) []const u8 {
        return switch (self) {
            ...
            .unclassified => "outside every known block — an index the decoder did not expect",
        };
    }
};
...
test "the backend block can be touched without a target being named" {
    var journal = Journal{};
    // A render-backend register that is not one of the three target registers.
    journal.record(0x2080, 0xFFFF_FFFF, 10);
