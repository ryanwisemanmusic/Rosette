...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-xenia-log-phrase-map.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "common-xenia-log-phrase-map: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-xenia-log-phrase-map: $target compile-only check"
...
echo "common-xenia-log-phrase-map: PASS"
