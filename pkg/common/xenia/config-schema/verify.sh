...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-xenia-config-schema.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "common-xenia-config-schema: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-xenia-config-schema: $target compile-only check"
...
echo "common-xenia-config-schema: PASS"
