...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-xenia-rosette-graphics-bridge.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "common-xenia-rosette-graphics-bridge: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-xenia-rosette-graphics-bridge: $target compile-only check"
...
echo "common-xenia-rosette-graphics-bridge: PASS"
