...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-xenia-fatal-condition-map.XXXXXX")"
...
echo "common-xenia-fatal-condition-map: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-xenia-fatal-condition-map: $target compile-only check"
...
echo "common-xenia-fatal-condition-map: PASS"
