...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-xenos-register-map.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "common-xenos-register-map: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-xenos-register-map: $target compile-only check"
...
echo "common-xenos-register-map: PASS"
