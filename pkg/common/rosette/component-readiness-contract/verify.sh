...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-rosette-component-readiness-contract.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "common-rosette-component-readiness-contract: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-rosette-component-readiness-contract: $target compile-only check"
...
echo "common-rosette-component-readiness-contract: PASS"
