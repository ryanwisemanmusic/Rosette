...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-rosette-host-capability-contract.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "common-rosette-host-capability-contract: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-rosette-host-capability-contract: $target compile-only check"
...
echo "common-rosette-host-capability-contract: PASS"
