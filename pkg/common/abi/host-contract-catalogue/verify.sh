...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-abi-host-contract-catalogue.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "common-abi-host-contract-catalogue: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-abi-host-contract-catalogue: $target compile-only check"
...
echo "common-abi-host-contract-catalogue: PASS"
