...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-abi-receiver-classification.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "common-abi-receiver-classification: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-abi-receiver-classification: $target compile-only check"
...
echo "common-abi-receiver-classification: PASS"
