...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-abi-near-null-shape.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "common-abi-near-null-shape: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-abi-near-null-shape: $target compile-only check"
...
echo "common-abi-near-null-shape: PASS"
