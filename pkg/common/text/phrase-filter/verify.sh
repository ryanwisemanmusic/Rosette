...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-text-phrase-filter.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "common-text-phrase-filter: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-text-phrase-filter: $target compile-only check"
...
echo "common-text-phrase-filter: PASS"
