...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/common-diagnostics-async-log-contract.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "common-diagnostics-async-log-contract: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "common-diagnostics-async-log-contract: $target compile-only check"
...
echo "common-diagnostics-async-log-contract: PASS"
