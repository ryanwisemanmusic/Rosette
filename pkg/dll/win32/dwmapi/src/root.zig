...
pub const dll_name = "dwmapi.dll";
pub const stem = "dwmapi";
...
test "DLL identity is case-insensitive and the degraded inventory is local" {
    try std.testing.expect(matches("dwmapi.dll"));
    try std.testing.expect(matches("dwmapi"));
    try std.testing.expect(!hasDegradedImport("__not_in_this_dll__"));
}

// --- generated export table: tools/dll/generate_export_tables.py ---
/// Every name this image imports from dwmapi.dll, with the ABI each return value
...
pub const exports = [_]export_contract.Export{
    .{ .name = "DwmSetWindowAttribute", .convention = .hresult, .behaviour = .modelled, .reviewed = true },
};
