...
pub const dll_name = "api-ms-win-crt-locale-l1-1-0.dll";
pub const stem = "api-ms-win-crt-locale-l1-1-0";
...
test "DLL identity is case-insensitive and the degraded inventory is local" {
    try std.testing.expect(matches("api-ms-win-crt-locale-l1-1-0.dll"));
    try std.testing.expect(matches("api-ms-win-crt-locale-l1-1-0"));
    try std.testing.expect(hasDegradedImport("___mb_cur_max_func"));
    try std.testing.expect(!hasDegradedImport("__not_in_this_dll__"));
}

// --- generated export table: tools/dll/generate_export_tables.py ---
/// Every name this image imports from api-ms-win-crt-locale-l1-1-0.dll, with the ABI each return value
...
pub const exports = [_]export_contract.Export{
    .{ .name = "___mb_cur_max_func", .arity = 0, .convention = .zero_count, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true },
    .{ .name = "localeconv", .convention = .handle, .behaviour = .modelled, .reviewed = true, .note = "returns a pointer to a CRT global the caller dereferences without checking; NULL is a guest crash" },
    .{ .name = "setlocale", .arity = 2, .convention = .handle, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "NULL means the locale could not be set, which callers probe for" },
};
