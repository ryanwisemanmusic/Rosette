...
pub const dll_name = "api-ms-win-crt-environment-l1-1-0.dll";
pub const stem = "api-ms-win-crt-environment-l1-1-0";
...
test "DLL identity is case-insensitive and the degraded inventory is local" {
    try std.testing.expect(matches("api-ms-win-crt-environment-l1-1-0.dll"));
    try std.testing.expect(matches("api-ms-win-crt-environment-l1-1-0"));
    try std.testing.expect(hasDegradedImport("__p__environ"));
    try std.testing.expect(!hasDegradedImport("__not_in_this_dll__"));
}

// --- generated export table: tools/dll/generate_export_tables.py ---
/// Every name this image imports from api-ms-win-crt-environment-l1-1-0.dll, with the ABI each return value
...
pub const exports = [_]export_contract.Export{
    .{ .name = "__p__environ", .convention = .handle, .behaviour = .modelled, .reviewed = true, .note = "returns a pointer to a CRT global the caller dereferences without checking; NULL is a guest crash" },
    .{ .name = "__p__wenviron", .convention = .handle, .behaviour = .modelled, .reviewed = true, .note = "returns a pointer to a CRT global the caller dereferences without checking; NULL is a guest crash" },
    .{ .name = "_wgetcwd", .convention = .zero_count, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "a POSIX-shaped status where zero is success and -1 is failure; the sign is the answer" },
    .{ .name = "getenv", .arity = 1, .convention = .handle, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "NULL means the variable is unset, which is the answer" },
};
