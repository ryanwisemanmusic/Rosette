...
pub const dll_name = "api-ms-win-crt-private-l1-1-0.dll";
pub const stem = "api-ms-win-crt-private-l1-1-0";
...
test "DLL identity is case-insensitive and the degraded inventory is local" {
    try std.testing.expect(matches("api-ms-win-crt-private-l1-1-0.dll"));
    try std.testing.expect(matches("api-ms-win-crt-private-l1-1-0"));
...
/// Every name this image imports from api-ms-win-crt-private-l1-1-0.dll, with the ABI each return value
...
pub const exports = [_]export_contract.Export{
    .{ .name = "longjmp", .convention = .void_call, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true },
    .{ .name = "memchr", .convention = .handle, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "NULL means not found, which is the answer" },
    .{ .name = "memcmp", .convention = .zero_count, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "a comparison or a length: zero means equal or empty, never failure" },
    .{ .name = "memcpy", .convention = .handle, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "returns its destination or NULL; the string family never sets errno" },
    .{ .name = "memmove", .convention = .handle, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "returns its destination or NULL; the string family never sets errno" },
    .{ .name = "strchr", .convention = .handle, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "NULL means not found, which is the answer" },
    .{ .name = "strrchr", .convention = .handle, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "NULL means not found, which is the answer" },
    .{ .name = "strstr", .convention = .handle, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "NULL means not found, which is the answer" },
};
