...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/dll-win32-return-contract.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "dll-win32-return-contract: native tests"
"$zig_bin" test \
...
for target in aarch64-freestanding x86_64-freestanding; do
    echo "dll-win32-return-contract: $target compile-only check"
...
echo "dll-win32-return-contract: PASS"
