...
pub const dll_name = "SHELL32.dll";
pub const stem = "shell32";
...
test "DLL identity is case-insensitive and the degraded inventory is local" {
    try std.testing.expect(matches("SHELL32.dll"));
    try std.testing.expect(matches("shell32"));
    try std.testing.expect(hasDegradedImport("ShellExecuteW"));
    try std.testing.expect(!hasDegradedImport("__not_in_this_dll__"));
}

// --- generated export table: tools/dll/generate_export_tables.py ---
/// Every name this image imports from SHELL32.dll, with the ABI each return value
...
pub const exports = [_]export_contract.Export{
    .{ .name = "CommandLineToArgvW", .arity = 2, .convention = .handle, .behaviour = .modelled, .reviewed = true },
    .{ .name = "DragAcceptFiles", .convention = .void_call, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true },
    .{ .name = "DragFinish", .convention = .void_call, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true },
    .{ .name = "DragQueryFileW", .arity = 4, .convention = .zero_count, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "returns characters copied, or the file count when the index is 0xFFFFFFFF" },
    .{ .name = "SHGetFolderPathW", .convention = .hresult, .behaviour = .modelled, .reviewed = true },
    .{ .name = "SHGetKnownFolderPath", .convention = .hresult, .behaviour = .modelled, .reviewed = true },
    .{ .name = "ShellExecuteW", .arity = 6, .convention = .zero_count, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "returns a pseudo-HINSTANCE; a value above 32 is success" },
};
