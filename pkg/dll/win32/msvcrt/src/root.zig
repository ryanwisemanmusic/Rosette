...
pub const dll_name = "msvcrt.dll";
pub const stem = "msvcrt";
pub const match_prefix = "";
pub const subsystem_name = "c_runtime";
...
test "DLL identity is case-insensitive and the degraded inventory is local" {
    try std.testing.expect(matches("msvcrt.dll"));
    try std.testing.expect(matches("msvcrt"));
    try std.testing.expect(hasDegradedImport("_snprintf"));
    try std.testing.expect(!hasDegradedImport("__not_in_this_dll__"));
}

// --- generated export table: tools/dll/generate_export_tables.py ---
/// Every name this image imports from msvcrt.dll, with the ABI each return value
...
pub const exports = [_]export_contract.Export{
    .{ .name = "_setjmp", .arity = 2, .convention = .zero_count, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "returns zero on the direct call and non-zero when longjmp resumes it - both are answers" },
    .{ .name = "_snprintf", .convention = .zero_count, .behaviour = .refused_by_policy, .zero_is_an_answer = true, .reviewed = true, .note = "returns characters written, or negative on error; zero is an empty result" },
};
