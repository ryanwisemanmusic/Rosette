...
/// Every name this image imports from ole32.dll, with the ABI each return value
...
pub const exports = [_]export_contract.Export{
    .{ .name = "CLSIDFromString", .convention = .hresult, .behaviour = .modelled, .reviewed = true },
    .{ .name = "CoCreateInstance", .convention = .hresult, .behaviour = .modelled, .reviewed = true },
    .{ .name = "CoInitializeEx", .convention = .hresult, .behaviour = .modelled, .reviewed = true },
    .{ .name = "CoTaskMemFree", .convention = .void_call, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true },
    .{ .name = "CoUninitialize", .convention = .void_call, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true },
    .{ .name = "PropVariantClear", .convention = .hresult, .behaviour = .modelled, .reviewed = true },
};
