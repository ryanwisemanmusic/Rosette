...
pub const dll_name = "api-ms-win-crt-heap-l1-1-0.dll";
pub const stem = "api-ms-win-crt-heap-l1-1-0";
...
test "DLL identity is case-insensitive and the degraded inventory is local" {
    try std.testing.expect(matches("api-ms-win-crt-heap-l1-1-0.dll"));
    try std.testing.expect(matches("api-ms-win-crt-heap-l1-1-0"));
    try std.testing.expect(hasDegradedImport("_aligned_realloc"));
    try std.testing.expect(!hasDegradedImport("__not_in_this_dll__"));
}

// --- generated export table: tools/dll/generate_export_tables.py ---
/// Every name this image imports from api-ms-win-crt-heap-l1-1-0.dll, with the ABI each return value
...
pub const exports = [_]export_contract.Export{
    .{ .name = "_aligned_free", .arity = 1, .convention = .void_call, .behaviour = .served, .zero_is_an_answer = true, .reviewed = true },
    .{ .name = "_aligned_malloc", .convention = .handle, .behaviour = .served, .reviewed = true },
    .{ .name = "_aligned_realloc", .convention = .handle, .behaviour = .modelled, .reviewed = true },
    .{ .name = "_set_new_mode", .convention = .zero_count, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true, .note = "a CRT bootstrap hook whose zero is success or is ignored entirely" },
    .{ .name = "calloc", .convention = .handle, .behaviour = .served, .reviewed = true },
    .{ .name = "free", .arity = 1, .convention = .void_call, .behaviour = .served, .zero_is_an_answer = true, .reviewed = true },
    .{ .name = "malloc", .convention = .handle, .behaviour = .served, .reviewed = true },
    .{ .name = "realloc", .convention = .handle, .behaviour = .served, .reviewed = true },
};
