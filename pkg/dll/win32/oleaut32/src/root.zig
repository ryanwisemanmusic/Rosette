...
pub const dll_name = "OLEAUT32.dll";
pub const stem = "oleaut32";
...
test "DLL identity is case-insensitive and the degraded inventory is local" {
    try std.testing.expect(matches("OLEAUT32.dll"));
    try std.testing.expect(matches("oleaut32"));
    try std.testing.expect(hasDegradedImport("SysFreeString"));
    try std.testing.expect(!hasDegradedImport("__not_in_this_dll__"));
}

// --- generated export table: tools/dll/generate_export_tables.py ---
/// Every name this image imports from OLEAUT32.dll, with the ABI each return value
...
pub const exports = [_]export_contract.Export{
    .{ .name = "SysFreeString", .arity = 1, .convention = .void_call, .behaviour = .modelled, .zero_is_an_answer = true, .reviewed = true },
};
