...
pub const dll_name = "bcrypt.dll";
pub const stem = "bcrypt";
...
test "DLL identity is case-insensitive and the degraded inventory is local" {
    try std.testing.expect(matches("bcrypt.dll"));
    try std.testing.expect(matches("bcrypt"));
    try std.testing.expect(!hasDegradedImport("__not_in_this_dll__"));
}

// --- generated export table: tools/dll/generate_export_tables.py ---
/// Every name this image imports from bcrypt.dll, with the ABI each return value
...
pub const exports = [_]export_contract.Export{
    .{ .name = "BCryptCloseAlgorithmProvider", .convention = .ntstatus, .behaviour = .modelled, .reviewed = true, .note = "STATUS_SUCCESS is zero; Xenia uses this only as an entropy source" },
    .{ .name = "BCryptGenRandom", .convention = .ntstatus, .behaviour = .modelled, .reviewed = true, .note = "STATUS_SUCCESS is zero; Xenia uses this only as an entropy source" },
    .{ .name = "BCryptOpenAlgorithmProvider", .convention = .ntstatus, .behaviour = .modelled, .reviewed = true, .note = "STATUS_SUCCESS is zero; Xenia uses this only as an entropy source" },
};
