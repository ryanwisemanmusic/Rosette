...
test "DLL identity is case-insensitive and the degraded inventory is local" {
    try std.testing.expect(matches("dxgi.dll"));
    try std.testing.expect(matches("dxgi"));
    try std.testing.expect(hasDegradedImport("CreateDXGIFactory1"));
    try std.testing.expect(!hasDegradedImport("__not_in_this_dll__"));
}

// --- generated export table: tools/dll/generate_export_tables.py ---
/// Every name this image imports from dxgi.dll, with the ABI each return value
...
pub const exports = [_]export_contract.Export{
    .{ .name = "CreateDXGIFactory1", .convention = .hresult, .behaviour = .modelled, .reviewed = true },
};
