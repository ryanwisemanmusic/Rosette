//! x86-64 host-side facts for the audio mixer and its callback.
//!
...
///
/// SSE2 is baseline in the x86-64 ABI, so a 128-bit mix needs no
/// runtime detection. Wider units exist but are not guaranteed, and Rosetta 2
/// does not provide AVX at all.
pub const mix_vector_bytes: u32 = 16;
pub const mix_samples_per_vector: u32 = mix_vector_bytes / mix_sample_bytes;
pub const mix_simd_extension = "sse2";

/// The control register carrying the flush-to-zero controls, and the bits that
/// turn them on.
pub const denormal_control_register = "mxcsr";
pub const denormal_control_bits: u32 = 0x8040;
...
test "package identity is the x86-64 audio route" {
    try std.testing.expectEqualStrings("x86_64", host_architecture);
    try std.testing.expectEqualStrings("sse2", mix_simd_extension);
    try std.testing.expectEqualStrings("mxcsr", denormal_control_register);
}
...
test "the denormal controls are a non-empty bit set in the route's register" {
    ...
    try std.testing.expectEqual(@as(u32, 0x8040), denormal_control_bits);
}
