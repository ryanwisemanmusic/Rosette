//! Package-backed selection facts for hot Xenia Mach-O primitive imports.
//! The primitive registry still owns handler execution and fallback behavior.
...
/// The order follows the existing primitive registry. Keeping the first-match
/// order here prevents a package hit from changing an ambiguous symbol's
