...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/xenia-x86-translation-progress.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "xenia-x86-translation-progress: native tests"
"$zig_bin" test \
    --cache-dir "$tmp_dir/native-cache" \
    --global-cache-dir "$tmp_dir/native-global-cache" \
    "$package_dir/src/root.zig"

echo "xenia-x86-translation-progress: x86_64 compile-only check"
...
echo "xenia-x86-translation-progress: PASS"
