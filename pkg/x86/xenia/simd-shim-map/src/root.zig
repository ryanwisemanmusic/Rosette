//! x86-64 host facts for Xenia's SIMD translation shims.
//!
...
///
/// True on this route: the host decodes VEX itself, so the AVX-to-NEON
/// shims are dead weight here.
pub const executes_vex_natively: bool = true;
...
test "package identity is the x86-64 SIMD route" {
...
test "the x86 route executes VEX natively and needs no AVX shim" {
    // On a genuine x86-64 host the AVX-to-NEON shims are dead weight: the
    // host decodes VEX itself. Recording that keeps a well-meaning change
    // from routing x86 through a translation layer it does not need.
...
test "the PowerPC shims stay live on every route" {
    // The guest is PowerPC whichever machine emulates it, so a route that
    // dropped these would have no way to express a VMX guest instruction.
    try std.testing.expect(isLive(.vmx_to_neon));
    try std.testing.expect(isLive(.ppc_float_to_neon));
}

test "AMX is not an x86 facility" {
    // Apple's matrix unit does not exist on this route.
    try std.testing.expect(!isLive(.amx_neon));
    try std.testing.expectEqualStrings("arm64", Shim.amx_neon.sourceIsa());
}
