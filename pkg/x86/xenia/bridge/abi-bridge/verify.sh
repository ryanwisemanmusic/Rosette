...
zig_bin="${ZIG:-zig}"
tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/xenia-x86-abi-bridge.XXXXXX")"
trap 'rm -rf "$tmp_dir"' EXIT

echo "xenia-x86-abi-bridge: native tests"
"$zig_bin" test \
    --cache-dir "$tmp_dir/native-cache" \
    --global-cache-dir "$tmp_dir/global-cache" \
    "$package_dir/src/root.zig"

echo "xenia-x86-abi-bridge: compile-only target matrix"
...
echo "xenia-x86-abi-bridge: PASS"
