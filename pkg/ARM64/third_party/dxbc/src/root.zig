//! Source mirror of `third_party/dxbc/dxbc_checksum.zig` for ARM64.
//! This records immutable shader-container facts; checksum execution remains runtime-owned.

const std = @import("std");

pub const host_architecture = "aarch64";
...
test "ARM64 DXBC ABI facts validate" {
    try std.testing.expect(host_is_little_endian);
    try std.testing.expect(validate());
    try std.testing.expectEqualStrings("aarch64", host_architecture);
}

test "ARM64 DXBC source mirror fingerprint is stable" {
