//! ARM64-route text ABI for successful Xenia PPC guest translation.
//!
...
/// Parse the stable, producer-neutral line emitted by Xenia's PPC frontend:
///
/// `READY COMPILER: translation-progress schema=1 generation=8 guest_function=0x82582cc8`
///
/// Fields are keyed rather than positional so log prefixes and severity text
/// may change without changing the ABI. Missing, malformed, or invalid fields
/// are rejected rather than treated as progress.
...
test "the ARM64 package identifies its native route" {
